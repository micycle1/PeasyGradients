...
/**
 * AKA JzAzBz https://observablehq.com/@jrus/jzazbz
 ...
 * 
 * Jzazbz is a a colour space designed for perceptul uniformity in high dynamic
 * range (HDR) and wide colour gamut (WCG) applications. Conceptually it is
 * similar to CIE L*a*b*, but has claimed improvements:
 * 
...
public final class JAB {
	...
	public static double[] jab2rgb(double[] jab) {
		return XYZ.xyz2rgbQuick(jab2xyz(jab));
	}
	...
	private static double[] xyz2jab(double[] xyz) {
		...
		for (int i = 0; i < 3; i++) {
			LMSp[i] = Math.pow(
					(c1 + (c2 * FastMath.pow(LMS[i] / 10000, n))) / (1 + (c3 * FastMath.pow(LMS[i] / 10000, n))), p);
		}
	...
	private static double[] jab2xyz(double[] jab) {
		...
		double[] LMS = new double[3];
		LMS[0] = 10000
				* FastMath.powQuick((c1 - FastMath.powQuick(LMSp[0], pInverse)) / ((c3 * FastMath.powQuick(LMSp[0], pInverse)) - c2), nInverse);
		LMS[1] = 10000
				* FastMath.powQuick((c1 - FastMath.powQuick(LMSp[1], pInverse)) / ((c3 * FastMath.powQuick(LMSp[1], pInverse)) - c2), nInverse);
		LMS[2] = 10000
				* FastMath.powQuick((c1 - FastMath.powQuick(LMSp[2], pInverse)) / ((c3 * FastMath.powQuick(LMSp[2], pInverse)) - c2), nInverse);
